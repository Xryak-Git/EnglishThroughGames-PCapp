...
def set_video_path(sender: str, app_data: dict):
...
def video_handle(video_params_root: int, *args, **kwargs):
...
def save_settings(sender, app_data, user_data: Settings):
    processes = dpg.get_value(item_id["labels"]["processes_number"])
    if _processes_valid(processes):
...
def set_processes(sender: str, app_data: str):
    if _processes_valid(app_data):
        return
    dpg.set_value(item_id["labels"]["processes_number"], "")


def set_positive_integer(sender: str, value: str):
...
def to_frames(sender: str, value: str):
...
def _processes_valid(processes: str):
